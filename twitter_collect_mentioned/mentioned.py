...
def get_mention_tweets():
    ...
    q = urllib.parse.urlencode({'q':'@bni','count': 100, 'tweet_mode':'extended'})
...
def insert_to_db(tweets):
    ...
    for tweet in tweets:
        ...
        if(cursor.rowcount == 1):
            ...
            query = f"""
            INSERT INTO tweet_posts(id, tweet_id, content, tweet_link, author, author_followers_count, author_statuses_count, author_verified,author_protected, posted_at, created_at, updated_at)
            VALUES(uuid_generate_v1(),'{tweet['tweet_id']}','{tweet['text']}','{tweet['tweet_link']}','{tweet['author_screen_name']}','{tweet['author_followers_count']}','{tweet['author_statuses_count']}','{tweet['author_verified']}','{tweet['author_protected']}','{tweet['posted_at']}',current_timestamp, current_timestamp)
            """
        cursor.execute(query)
        connection.commit()
